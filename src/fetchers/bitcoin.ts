...
export async function fetchNwsInfo(): Promise<{ price: number; change: number } | undefined> {
  ...
  try {
    ...
    return Math.abs(change) > config.bitcoin.significantChangeThreshold ? { price, change } : undefined;
  } catch (error) {
